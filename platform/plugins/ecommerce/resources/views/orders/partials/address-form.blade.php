<script>
    function allowAlphabetsOnly(event) {
        ...
        // Remove non-alphabet characters from input
        inputField.value = inputField.value.replace(/[^A-Za-z]/g, '');
    }

    function validateSixDigitNumber(event) {
        const inputField = event.target;
        const errorMessage = document.getElementById('error-message');
        const regex = /^\d{0,6}$/; // Allows up to 6 digits only

        if (!regex.test(inputField.value)) {
            // Trim the value to match only the first 6 digits
            inputField.value = inputField.value.replace(/[^\d]/g, '').substring(0, 6);
            errorMessage.textContent = "Only six-digit numbers are allowed.";
        } else if (inputField.value.length > 6) {
            inputField.value = inputField.value.substring(0, 6); // Enforce max length
            errorMessage.textContent = "Maximum 6 digits allowed.";
        } else {
            ...
            <div class="form-group mb-3 @error('address.zip_code') has-error @enderror">
                <div class="form-input-wrapper">
                    <input
                            ...
                            type="text"
                            id="sixDigitNumber"
                            oninput="validateSixDigitNumber(event)"
                    >
