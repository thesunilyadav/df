...
namespace Botble\Surepay\Services\Abstracts;

use Botble\Payment\Services\Traits\PaymentErrorTrait;
use Botble\Surepay\Services\Surepay;
use Botble\Support\Services\ProduceServiceInterface;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Arr;

abstract class SurepayPaymentAbstract implements ProduceServiceInterface
{
    ...
    public function getPaymentDetails($payment)
    {
        try {
            ...
            $response = (new Surepay())->getListTransactions($params);
            if ($response['status']) {
    ...
    {
        try {
            $response = (new Surepay())->refundOrder($paymentId, $amount);
    ...
    {
        try {
            $response = (new Surepay())->getRefundDetails($refundId);
            if ($response['status']) {
                return [
                    'error' => false,
                    'message' => $response['message'],
