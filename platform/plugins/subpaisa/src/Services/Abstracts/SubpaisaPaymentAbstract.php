...
namespace Botble\Subpaisa\Services\Abstracts;

use Botble\Payment\Services\Traits\PaymentErrorTrait;
use Botble\Subpaisa\Services\Subpaisa;
use Botble\Support\Services\ProduceServiceInterface;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Arr;

abstract class SubpaisaPaymentAbstract implements ProduceServiceInterface
{
    ...
    public function getPaymentDetails($payment)
    {
        try {
            ...
            $response = (new Subpaisa())->getListTransactions($params);
            if ($response['status']) {
    ...
    {
        try {
            $response = (new Subpaisa())->refundOrder($paymentId, $amount);
    ...
    {
        try {
            $response = (new Subpaisa())->getRefundDetails($refundId);
            if ($response['status']) {
                return [
                    'error' => false,
                    'message' => $response['message'],
